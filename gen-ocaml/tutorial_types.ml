...
object (self)
  val mutable _what : Int32.t option = None
  method get_what = _what
  method grab_what = match _what with None->raise (Field_empty "invalidOperation.what") | Some _x10 -> _x10
  method set_what _x10 = _what <- Some _x10
  method unset_what = _what <- None
  method reset_what = _what <- None
  ...
  method write (oprot : Protocol.t) =
    ...
    (match _what with None -> () | Some _v -> 
      oprot#writeFieldBegin("what",Protocol.T_I32,1);
      oprot#writeI32(_v);
      oprot#writeFieldEnd
    );
